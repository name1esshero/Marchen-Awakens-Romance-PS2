...
- 214 accessors (1,712 bytes) across thirteen distinct classes (`CCamera`,
  `CCamera2`, `CCameraMv`, `CRender`, `CRender2`, `CGameCamera`, `C3dObject`,
  `CChara`, `CCharaBase`, `CWeapon`, `CMotion3`, `CMotion`, `CGameCntrl`) have
  verified assembly implementations.
- EE GCC `2.96-ee-001003-1` with the same single `-O2` flag compiles natural
  C++ candidates for all 214 into the same sections. Substituting them
...
- **3,443,492 bytes** of the boot ELF remain explicitly preserved as unrecovered
...
- Complete class inheritance, virtual layout and size are unknown for all
  thirteen partial classes above. Matching these small accessors is
  insufficient to promote the classes to recovered source. 519 more
