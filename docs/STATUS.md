...
- One 20,452-byte `_msg.dat` message table has a strict editable JSON
  ...
  reusable `make verify-graphics-image` gate reparses all six English TXCs
  through ISO/YFS/BPE/UI tables and confirms 709,120 bytes exactly match staged
  ...
  `reports/mar_eng_compare.json` and
  [`GRAPHIC_TEXT_LOCALIZATION.md`](../tasks/GRAPHIC_TEXT_LOCALIZATION.md).
  The current ISO reparse confirms resource sizes of 20,714 bytes for `_msg.dat`,
  14,120 bytes for `CardList.txt`, and 3,320 bytes for `DataBase.txt`; all match
  their tracked catalog-generated bytes. The 142-row card
