...
override.
Verification: the graphics audit recognized two English title overrides and
reported zero changed Japanese baselines. The generated main-logo TXC matched
the staged override exactly after export and reparse through ISO/YFS/BPE/UI-table
layers; the three text catalogs also matched their generated bytes in the same
mod image.
Scope: the `title_marh` and `sbttl` title atlases in this game's UI bundle.
Limits: reusing the wordmark does not establish the correct on-screen crop,
scale, animation state or draw order. The sibling texture still displays the
English mark elsewhere, so duplicate branding is possible until AT/UV layout
and runtime rendering are checked.
