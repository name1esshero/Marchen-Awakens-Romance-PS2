...
| P1 | Decode and localize UI graphics | All 724 menu `.b` leaves decode to declared sizes. Of these, 721 use a validated 32-byte resource table: 3,084 members were extracted and all 721 untouched bundles rebuild byte-exactly. The root `graphics/` workspace exports 1,457 editable menu TGAs (1,226 PSMT8, 231 PSMT4) directly into flat human-readable categories; immutable `*_jp.tga` baselines pair with editable `*_eng.tga` siblings, which override them for `mar_eng.iso`. Indexed conversion uses linear pixels, mapped PSMT8 CLUT and expanded GS alpha. The title view matches the owner-supplied reference. Source hashes/dimensions audited cleanly; staging an edited image is covered through nested UI-table/BPE/PAC rebuild tests. Only a subset of the full TXC census is exposed; AT animation bodies, UV composition and runtime display remain unresolved. See [title rendering evidence](../tasks/TITLE_TEXTURE_RENDERING.md). | Decode the AT animation body and correlate UVs with the draw path; create/review an English variant, reinsert and relocate, then visually/runtime validate where supported; inventory alternate `.yma` structures |
...
| P1 | Asset workspace and translation surfaces | Full-disc unchanged rebuild matches; tracked catalogues cover `_msg.dat`, `CardList.txt`, and `DataBase.txt`. UI `.b` wrappers decode; 721 resource tables and 3,084 members have exact no-op reassembly and edited growth/repoint support; 3 alternate `.yma` layouts remain raw. The root `graphics/` workspace has 1,457 editable TGAs in flat semantic categories, with source hashes and container paths in its index. Immutable Japanese `*_jp.tga` baselines pair with same-size English `*_eng.tga` siblings selected during `make build-mod-disc`; staged resources go under `build/graphics-overrides/`. Runtime display has not been verified. `make build-mod-disc` writes `mar_eng.iso` in the workspace root for emulator testing. | Translate CardList captions and investigate database condition metadata; decode AT animation composition, translate a selected graphic, validate in runtime, and improve append placement; see [method](TASK_ASSET_WORKSPACE_METHODOLOGY.md) |
