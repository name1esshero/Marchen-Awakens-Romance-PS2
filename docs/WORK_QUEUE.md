...
| P1 | Decode and localize UI graphics | 724 menu `.b` leaves are candidate graphics; sampled `000_top_at.b` begins with `BPE\0`, but payload format, text-bearing content and exact codec are unresolved. Next inspect headers/known consumers, decode one representative reversibly, replace one text-bearing graphic with English, then reinsert and reparse it through PAC/YFS/ISO. | Byte-exact decode/re-encode for untouched sample, controlled edited asset growth/relocation, and visual/runtime validation where supported; preserve unknown bytes and format claims as candidates until evidenced |
