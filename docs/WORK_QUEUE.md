...
| P1 | Asset workspace and translation surfaces | Full-disc unchanged rebuild matches; six CP932/UTF-8 companions are exposed. Synthetic growing UTF-8 translation now traverses CP932 encoding and ISO relocation; `make build-mod-disc` is available. Catalog tables are the only confirmed player-facing text candidates so far; UI `.b` and `_msg.dat` still need format recovery. | Edit and validate a real catalog/UI text leaf, identify remaining text-bearing formats, and run runtime checks; see [method](TASK_ASSET_WORKSPACE_METHODOLOGY.md) |
