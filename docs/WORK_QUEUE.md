...
| P1 | Decode and localize UI graphics | All 724 menu `.b` leaves decode to declared sizes. Of these, 721 use a validated 32-byte resource table: 3,084 members were extracted and all 721 untouched bundles rebuild byte-exactly. TGA import/export now handles RTX3 PSMT4/PSMT8/PSMCT32, same-dimension edits and existing palettes; high-bit modes are withheld pending layout evidence. Atlas/UV composition remains unresolved. | Recover AT/UV composition, identify a text-bearing graphic, create/review an English variant, reinsert and relocate, then visually/runtime validate where supported; inventory alternate `.yma` structures |
