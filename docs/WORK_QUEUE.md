...
| P1 | Asset workspace and translation surfaces | Full-disc unchanged rebuild matches; six CP932/UTF-8 companions and one 229-entry `_msg.dat` source catalog are exposed. One save/start prompt has an initial English rendering and passed catalog-driven full ISO/YFS/PAC growth/reparse checks. UI `.b` resources remain opaque. | Review/complete translation, validate in runtime, identify remaining UI text, and improve append placement; see [method](TASK_ASSET_WORKSPACE_METHODOLOGY.md) |
