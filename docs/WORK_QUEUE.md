...
| P1 | Decode and localize UI graphics | All 724 menu `.b` leaves decode to declared sizes. Of these, 721 use a validated 32-byte resource table: 3,084 members were extracted and all 721 untouched bundles rebuild byte-exactly. TGA import/export handles RTX3 PSMT4/PSMT8/PSMCT32; `--stored-order` exposes indexed bytes for diagnosis. The title TXC hash matches its bundle and all four AT texture-name references match TXC members. AT animation bodies and UV composition remain unresolved. See [title rendering evidence](../tasks/TITLE_TEXTURE_RENDERING.md). | Decode the AT animation body and correlate UVs with the draw path; create/review an English variant, reinsert and relocate, then visually/runtime validate where supported; inventory alternate `.yma` structures |
