...
| P1 | Asset workspace and translation surfaces | Full-disc unchanged rebuild matches; six CP932/UTF-8 companions and one 229-entry `_msg.dat` JSON surface are exposed. A longer test message rebuilt through ISO/YFS/PAC, passed nested parse checks, and compared against the authenticated baseline. UI `.b` resources remain opaque. | Add a tracked original/translation catalog, validate in runtime, identify remaining UI text, and improve append placement; see [method](TASK_ASSET_WORKSPACE_METHODOLOGY.md) |
| P2 | Full-disc reconstruction and relocation | Untouched image matches exactly. A 24-byte `_msg.dat` test growth built a 5.016 GB ISO, passed ISO inventory and nested YFS/PAC message reparse, and produced an authenticated expected baseline mismatch. Current append path adds the full 428,967,936-byte YFS. | Runtime acceptance, broader relocation cases and a more space-efficient placement strategy |
